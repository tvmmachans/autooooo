...
export class GoogleTrendsService {
     ...
     */
    async getDailyTrends(region = 'IN', language = 'en') {
        try {
        ...
        try {
            ...
            if (data.default?.timelineData) {
                ...
                return {
                    ...
                    relatedQueries: data.default.relatedQueries?.map((q) => q.query) || [],
                };
            }
            return null;
        }
     ...
     */
    async saveTrends(trends, sourceId, region, language) {
    ...
    async getCachedTrends(region, language) {
        // Get from database cache
        const source = await db.select()
            .from(trendSources)
            .where(and(eq(trendSources.name, 'google_trends'), eq(trendSources.region, region), eq(trendSources.language, language)))
            .limit(1);
        if (source.length > 0) {
