...
export class TrendAnalysisService {
     ...
     */
    async calculateTrendScore(trendId) {
        ...
        // Weighted scoring
        const score = ((this.normalizeVolume(volume) * 0.4) +
     ...
     */
    async getRegionalTrends(region, language, limit = 20) {
        const trends = await db.select()
            .from(trendData)
            .where(and(eq(trendData.isRegional, region.includes('-KL') || region.includes('-TN')), eq(trendData.isActive, true)))
     ...
     */
    async trackCompetitorTrends(competitorHandles, platform) {
     ...
     */
    async detectSeasonalPattern(trendId) {
        const history = await this.getTrendHistory(trendId);
        if (history.length < 12)
            return false; // Need at least 12 months of data
    ...
    predictTrendEnd(history) {
        ...
        if (trend >= 0)
            return undefined; // Still growing
     ...
     */
    calculateFreshnessScore(firstSeen, lastUpdated) {
     ...
     */
    async recordTrendSnapshot(trendId) {
        const [trend] = await db.select()
            .from(trendData)
            .where(eq(trendData.id, trendId))
            .limit(1);
