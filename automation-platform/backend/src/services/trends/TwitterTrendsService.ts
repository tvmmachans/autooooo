...
export class TwitterTrendsService {
   ...
   */
  async getTrendingTopics(woeid: number = 23424848): Promise<TwitterTrend[]> {
   ...
   */
  async getIndiaTrends(): Promise<TwitterTrend[]> {
   ...
   */
  async searchKeyword(keyword: string): Promise<TwitterTrend | null> {
    if (!this.bearerToken) {
      return null;
    }
  ...
  private processTrends(data: any): TwitterTrend[] {
  ...
  private categorizeTrend(keyword: string): string {
  ...
  private async getCachedTrends(woeid: number): Promise<TwitterTrend[]> {
    const region = woeid === 23424848 ? 'IN' : 'IN-KL';
    const source = await db.select()
      .from(trendSources)
      .where(
        and(
          eq(trendSources.name, 'twitter'),
