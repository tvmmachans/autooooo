...
export class RegionalTrendsService {
     ...
     */
    calculateRelevanceScore(keyword, language, region) {
     ...
     */
    async getCachedRegionalTrends(region, language) {
        const sources = await db.select()
            .from(trendSources)
            .where(and(eq(trendSources.region, region), eq(trendSources.language, language), eq(trendSources.isActive, true)));
        const allTrends = [];
        for (const source of sources) {
            const trends = await db.select()
                .from(trendData)
                .where(and(eq(trendData.sourceId, source.id), eq(trendData.isActive, true)))
...
//# sourceMappingURL=RegionalTrendsService.js.map