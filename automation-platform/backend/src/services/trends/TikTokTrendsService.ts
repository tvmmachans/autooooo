...
export class TikTokTrendsService {
   ...
   */
  async getTrendingHashtags(region: string = 'IN'): Promise<TikTokTrend[]> {
    if (!this.accessToken) {
   ...
   * Get trending videos
   */
  async getTrendingVideos(region: string = 'IN'): Promise<TikTokTrend[]> {
    if (!this.accessToken) {
      return this.getCachedTrends(region);
    }
  ...
  private processHashtags(data: any): TikTokTrend[] {
  ...
  private processVideos(data: any): TikTokTrend[] {
    // Extract hashtags from trending videos
    const hashtagMap = new Map<string, { count: number; views: number }>();
  ...
  private async getCachedTrends(region: string): Promise<TikTokTrend[]> {
    const source = await db.select()
      .from(trendSources)
      .where(
        and(
          eq(trendSources.name, 'tiktok'),
