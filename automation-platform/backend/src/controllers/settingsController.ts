...
export class SettingsController {
    ...
    try {
      ...
      const [existing] = await db
        ...
        .where(and(
          ...
          eq(integrationTokens.platform, platform)
        ))
        .limit(1);
  ...
  async disconnectIntegration(req: Request, res: Response) {
    try {
      ...
      const [integration] = await db
        ...
        .from(integrationTokens)
        .where(and(
          eq(integrationTokens.id, parseInt(id)),
          eq(integrationTokens.userId, req.user.id)
        ))
        .limit(1);
  ...
  async getAIServiceConfig(req: Request, res: Response) {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Not authenticated' });
      }
  ...
  async updateAIServiceConfig(req: Request, res: Response) {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Not authenticated' });
      }
      ...
      const [existing] = await db
        ...
        .from(aiServiceConfig)
        .where(and(
          eq(aiServiceConfig.userId, req.user.id),
          eq(aiServiceConfig.service, service)
        ))
        .limit(1);
  ...
  async getWorkflowPreferences(req: Request, res: Response) {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Not authenticated' });
      }
  ...
  async updateWorkflowPreferences(req: Request, res: Response) {
    try {
      if (!req.user) {
        return res.status(401).json({ error: 'Not authenticated' });
      }
