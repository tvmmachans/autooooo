...
import type { Workflow, NewWorkflow, Node } from '../database/schema.js';
...
export class WorkflowController {
  ...
  async getWorkflows(req: Request, res: Response) {
    try {
      ...
      if (nodeType) {
        conditions.push(
          sql`${workflows.nodes}::jsonb @> ${JSON.stringify([{ type: nodeType }])}`
        );
      }
      ...
      const workflowList = await db
        .select({
          ...
          nodeTypes: sql<string[]>`array_agg(distinct elem->>'type') filter (where elem->>'type' is not null)`,
        })
        .from(workflows)
  ...
  async createWorkflow(req: Request, res: Response) {
    try {
      ...
      const { name, description, nodes, edges }: NewWorkflow = req.body;
      ...
      const hasStartNode = nodes.some((node: Node) => node.type === 'start');
      if (!hasStartNode) {
  ...
  async updateWorkflow(req: Request, res: Response) {
    try {
      ...
      const { name, description, nodes, edges, isActive }: Partial<NewWorkflow> = req.body;
  ...
  async searchWorkflowsByNodeProperties(req: Request, res: Response) {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Unauthorized' });
      ...
      if (nodeType || nodeData) {
        ...
        conditions.push(
          sql`${workflows.nodes}::jsonb @> ${JSON.stringify([jsonQuery])}`
        );
      }
