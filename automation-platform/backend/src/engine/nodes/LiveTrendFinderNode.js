...
export class LiveTrendFinderNode {
    ...
    async saveTrends(trends, region, language, platforms) {
        // Get or create trend source
        for (const platform of platforms) {
            let [source] = await db.select()
                .from(trendSources)
                .where(and(eq(trendSources.name, platform), eq(trendSources.region, region), eq(trendSources.language, language)))
                .limit(1);
            if (!source) {
            ...
            if (platform === 'google_trends') {
                await this.googleTrends.saveTrends(trends.filter(t => t.sources?.includes('google_trends') || !t.sources), source.id, region, language);
            }
            else if (platform === 'youtube') {
                await this.youtubeTrends.saveTrends(trends.filter(t => t.sources?.includes('youtube')), source.id, region);
            }
            // Add other platforms similarly...
        }
    }
}
//# sourceMappingURL=LiveTrendFinderNode.js.map