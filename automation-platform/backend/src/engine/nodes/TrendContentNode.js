...
export class TrendContentNode {
    ...
    async execute(context, input) {
        try {
            ...
            const contentResults = await Promise.all(selectedTrends.map(async (trend) => {
    ...
    async getTrendingTopics(platform, region, language, category) {
        // Check cache first
        const cached = await db.select()
            .from(trendCache)
            .where(and(eq(trendCache.platform, platform), eq(trendCache.region, region), eq(trendCache.language, language), category ? eq(trendCache.category, category) : undefined, gte(trendCache.expiresAt, new Date())))
            .limit(1);
        if (cached.length > 0 && cached[0].trends) {
            return cached[0].trends;
        }
    ...
    async fetchTrendsFromAPI(platform, region, language, category) {
...
//# sourceMappingURL=TrendContentNode.js.map