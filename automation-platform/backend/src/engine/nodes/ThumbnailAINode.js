...
export class ThumbnailAINode {
    ...
    generateThumbnailPrompt(title, description) {
    ...
    async extractThumbnail(videoPath, config) {
        ...
        await execAsync(`${ffmpegPath} -i "${videoPath}" -ss ${time} -vframes 1 "${thumbnailPath}"`);
    ...
    async addTextOverlay(thumbnailPath, text, config) {
...
//# sourceMappingURL=ThumbnailAINode.js.map