...
export class DatabaseUtils {
  ...
  // Workflow queries
  async getWorkflowsByUserId(userId: number, activeOnly = true) {
  ...
  async searchWorkflows(userId: number, searchTerm: string) {
    return this.db.select().from(schema.workflows)
      .where(and(
        eq(schema.workflows.userId, userId),
        or(
          like(schema.workflows.name, `%${searchTerm}%`),
          like(schema.workflows.description, `%${searchTerm}%`)
        )
      ));
  }

  // Execution queries
  async getExecutionsByWorkflowId(workflowId: number, limit = 50) {
  ...
  async getExecutionsByStatus(status: string, limit = 100) {
    return this.db.select()
      .from(schema.executions)
  ...
  async getRecentExecutions(userId: number, days = 7) {
    ...
    .where(and(
      eq(schema.workflows.userId, userId),
      gte(schema.executions.createdAt, cutoffDate)
    ))
    .orderBy(desc(schema.executions.createdAt));
  }

  // Execution logs queries
  async getExecutionLogs(executionId: number, limit = 1000) {
    return this.db.select()
  ...
  async findWorkflowsByNodeType(userId: number, nodeType: string) {
    return this.db.select()
      .from(schema.workflows)
      .where(and(
        eq(schema.workflows.userId, userId),
        eq(schema.workflows.isActive, true),
        sql`${schema.workflows.nodes}::jsonb @> ${JSON.stringify([{ type: nodeType }])}`
      ));
  }

  async searchExecutionHistory(workflowId: number, conditions: Record<string, any>, limit = 100) {
  ...
  async bulkInsertExecutionLogs(logs: schema.NewExecutionLog[]) {
  ...
  async loadWorkflowWithNodes(workflowId: number) {
    return this.db.select({
  ...
  async getExecutionContext(executionId: number) {
  ...
  async getCredentialsByType(userId: number, type: string) {
    return this.db.select()
  ...
  // Webhooks queries
  async getWebhooksByUserId(userId: number, activeOnly = true) {
  ...
  async getWebhooksByWorkflowId(workflowId: number) {
    return this.db.select()
  ...
  // Statistics queries
  async getWorkflowStats(userId: number) {
    const result = await this.db.select({
      totalWorkflows: sql<number>`count(*)`,
      activeWorkflows: sql<number>`count(case when ${schema.workflows.isActive} then 1 end)`,
    })
    .from(schema.workflows)
  ...
  async getExecutionStats(workflowId: number) {
    const result = await this.db.select({
      ...
      successfulExecutions: sql<number>`count(case when ${schema.executions.status} = 'completed' then 1 end)`,
      failedExecutions: sql<number>`count(case when ${schema.executions.status} = 'failed' then 1 end)`,
      runningExecutions: sql<number>`count(case when ${schema.executions.status} = 'running' then 1 end)`,
    })
    .from(schema.executions)
    .where(eq(schema.executions.workflowId, workflowId));
  ...
  async getSystemStats() {
    const [userStats] = await this.db.select({
      ...
      activeUsers: sql<number>`count(case when ${schema.users.isActive} then 1 end)`,
    }).from(schema.users);
    ...
    const [executionStats] = await this.db.select({
      totalExecutions: sql<number>`count(*)`,
      completedExecutions: sql<number>`count(case when ${schema.executions.status} = 'completed' then 1 end)`,
      failedExecutions: sql<number>`count(case when ${schema.executions.status} = 'failed' then 1 end)`,
    }).from(schema.executions);
