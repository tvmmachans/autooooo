...
export const AnimatedNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  return (
    <motion.div
      className={cn(
        'px-4 py-3 rounded-lg shadow-lg border-2 min-w-[150px]',
        `bg-gradient-to-r ${colorClass}`,
        selected ? 'border-blue-400 ring-2 ring-blue-400/50' : 'border-transparent'
      )}
    ...
    >
      ...
      <div className="text-white font-semibold text-sm">{data.label || data.type}</div>
      {data.description && (
